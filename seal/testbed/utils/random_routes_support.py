...
def set_options(
    ...
    optParser.add_option("-b", "--begin", type="float",
                         default=0, help="begin time")
    optParser.add_option("-e", "--end", type="float",
                         default=3600, help="end time (default 3600)")
    optParser.add_option(
    ...
    if options.period <= 0:
        my_log("Error: Period must be positive", file=sys.stderr)
        sys.exit(1)

    if options.jtrrouter and options.flows <= 0:
        my_log(
            "Error: Option --jtrrouter must be used with option --flows", file=sys.stderr)
        sys.exit(1)
...
def get_options(rrs_args=None, dir=""):
    ...
    if options.vehicle_class:
        ...
        if "type=" in options.tripattrs:
            my_log("Error: trip-attribute 'type' cannot be used together with option "
...
class CustomRandom(object):
    ...
    def __init__(self, generator, seed):
        if generator is None:
            my_log("Empty generator given")
            sys.exit(0)

        self.generator = generator
        if self.generator == 'default':
            if seed is None:
                my_log("Empty seed given")
                sys.exit(0)
            random.seed(seed)

    def random(self):
        if self.generator == 'default' or self.generator == "uniform":
            return random.random()
        elif self.generator == 'zipf':
            return np.random.zipf(2., None)
        # elif self.generator == 'uniform':
        #     return np.random.uniform(0., 1., None)
        elif self.generator == 'arcsine':
            return st.arcsine.rvs()


class RandomEdgeGenerator(object):
    ...
    def get(self):
        ...
        r = randomGenerator.random() * self.total_weight
...
def get_prob_fun(options, fringe_bonus, fringe_forbidden, max_length):
    ...
    def edge_probability(edge):
        ...
        if options.angle_weight != 1.0 and fringe_bonus is not None:
            ...
            # my_log("e=%s nc=%s ec=%s ea=%s a=%s ad=%s" % (
...
def buildTripGenerator(net, options):
    try:
        ...
        source_generator = RandomEdgeGenerator(
            net,
            get_prob_fun(options, "_incoming",
                         forbidden_source_fringe, max_length)
        )
        sink_generator = RandomEdgeGenerator(
            net,
            get_prob_fun(options, "_outgoing",
                         forbidden_sink_fringe, max_length)
        )
        ...
        my_log("Error: no valid edges for generating source or destination. "
    ...
    return RandomTripGenerator(source_generator,
                               sink_generator,
                               via_generator,
...
def split_trip_attributes(tripattrs, pedestrians, hasType):
    ...
    for a in tripattrs.split():
        if "=" in a:
            ...
            if len(allattrs) == 0:
                my_log("Warning: invalid trip-attribute '%s'" % a)
            else:
                allattrs[-1] += ' ' + a
...
def main(options):
    ...
    if options.min_distance > net.getBBoxDiameter() * (options.intermediate + 1):
        ...
        my_log(f"Warning: setting number of intermediate waypoints to "
