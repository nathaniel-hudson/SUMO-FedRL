...
def buildTripGenerator(net, options):
    try:
        ...
        my_log("Error: no valid edges for generating source or destination. Try using option --allow-fringe",
               file=sys.stderr)
        return None
...
def main(options):
    ...
    if options.min_distance > net.getBBoxDiameter() * (options.intermediate + 1):
        ...
        my_log(("Warning: setting number of intermediate waypoints to %s to achieve a minimum trip length of " +
                ...
                while depart < maxTime:
                    if options.binomial is None:
                        ...
                        for _ in range(options.binomial):
                            # TODO: Add feature such that `random.random()` can take on
                            # different kinds of random distributions.
